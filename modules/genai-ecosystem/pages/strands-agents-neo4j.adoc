...
[source,bash]
----
pip install strands-agents mcp neo4j pandas matplotlib seaborn openai
----
...
This example demonstrates a Strands Agent connecting directly to the official `mcp-neo4j-cypher` server using `uvx`. It connects to the public **Companies** demo database.
...
# This uses 'uvx' to run the official mcp-neo4j-cypher package directly
...
def main():
    ...
    with client:
        ...
        # 5) Ask a question
        query = "Find the company 'Patagonia' and tell me who its competitors are."
        print(f"\nUser Query: {query}")
