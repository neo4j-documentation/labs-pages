...
For using LlamaIndex as an agent framework with Neo4j — including MCP integration, `FunctionAgent`, `TextToCypherRetriever` as agent tools, and the Neo4j Query Engine tool spec — see xref:genai-frameworks/llamaindex-agents.adoc[LlamaIndex Agent Integration].
...
* https://neo4j.com/blog/knowledge-graph/knowledge-graph-agents-llamaindex/[Build Knowledge Graph Agents With LlamaIndex Workflows^]